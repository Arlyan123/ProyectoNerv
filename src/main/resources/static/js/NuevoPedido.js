...
btnAgregarCliente.addEventListener("click",function(){
	  ...
	  if ($("input[name=nombre]:checked").val() == null  ) {
		  swal("NERV", "Seleccione un cliente");
	      return ;
	  }
});
			    	  ...
			    	  $(document).on("click","a.eliminar",function(){
			    		  swal({
					    	   ...
					    	   if (willDelete) {
					    		    ...
					    		    swal("¡El zapato no ha sido eliminado!");
					    	   }
				    	   });	 
			    	  });
				  }
...
btnGuardarVenta.addEventListener("click",function(){
	var fecha = new Date();
	  ...
	  if ($("#codigoCliente").val() <= 0) {         
        swal("NERV", "Ingrese un cliente para registrar una venta");
        return;
	  }
	  if($("#totalVenta").val() <=0){
		  swal("NERV", "Ingrese como mínimo un zapato para registrar una venta");
		  return;
	  }
	  else{
		  //Creacion del json
